...
function showFooter (body, content, meta) {
  ...
  footer.innerHTML =
    ...
    // (meta.author || 'Unknow') + '. ' +
    '</a>Edited by <a href="http://www.gnu.org/software/emacs/">Emacs</a> or Others.' +
...
function genCategories (body, content) {
  ...
  if (orgUl) {
	...
	_tabContent.push(
	  a.innerHTML.replace(
	      /(.*) (\d{4}-\d{2}-\d{2})/,
	    '<li class="link"><a href="'+ a.getAttribute('href') + '">$1</a><p class="date">Last Modification Time: $2</p></li>'))
      })
